...
// `threads` > 1 parallelises over image columns; every output element is
// computed by exactly the same operations in the same order regardless of
// the thread count.
ImageD padarrayXT_symmetric(const ImageD& a, int p, int threads = 1);
...
ImageD conv2_sep_valid(const std::vector<double>& hcol, const std::vector<double>& hrow, const ImageD& a, int threads = 1);
